	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(sessionUser != null){
			...
			if(bankAccount !=null){
				...
				//These are from the jars we added in the lib forlder
				ObjectMapper mapper = new ObjectMapper(); 
				
				String json = mapper.writeValueAsString(userDTO);
				
				PrintWriter out = response.getWriter();
				response.setContentType("application/json"); //not plain text. type of content. developer defined
				out.write(json);
			}else{
				response.setStatus(418); //means I'm a teapot
			}
		}else{
